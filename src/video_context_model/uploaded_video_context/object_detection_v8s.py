...
def log_object_detection(objects, log_file="logs/object_detection_log.txt"):
    """Log object detection results with timestamp"""
...
def extract_objects_info(detection_result):
    """Extract and format object detection information"""
    objects = []
...
def analyze_scene_objects(objects):
    """Analyze detected objects for scene understanding"""
    if not objects:
...
def main():
    ...
    while True:
        ...
        # Log object detection every 30 frames (roughly every second at 30fps)
        if frame_count % 30 == 0:
        ...
        if len(frame_buffer) == CLIP_LEN and action_model:
            ...
            # Log action every time it's detected
        ...
        # Display scene info (every 30 frames)
